...
dS = zeros(nb,2);
dS(:,1) = -sqrt(pi/4./b).*exp(-b.*X(2))./X(1).^2.*erf(X(1).*sqrt(b))+...
    exp(-b.*X(2)-b.*X(1).^2);
dS(:,2) = sqrt(pi/4./b)./X(1).*erf(X(1).*sqrt(b)).*exp(-b.*X(2)).*(-b);
...
S = sqrt(pi/4./b)./X(1).*erf(X(1).*sqrt(b)).*exp(-b.*X(2));
% S = X(1).*sqrt(pi/4./b).*exp(-b.*X(3)/scale);
